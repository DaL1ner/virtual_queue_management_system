...
## Матрица доступа к эндпоинтам Ticket

### Группа эндпоинтов `/api/tickets`

| # | HTTP Метод | Endpoint | Описание | Аутентификация | Разрешённые роли |
|---|------------|----------|----------|----------------|------------------|
| 1 | `POST` | `/api/tickets/` | Создать новый талон | ❌ Не требуется | Все (публичный) |
| 2 | `GET` | `/api/tickets/{ticketId}` | Получить детальную информацию о талоне по ID | ✅ Требуется | ADMIN, OPERATOR |
| 3 | `POST` | `/api/tickets/{ticketId}/cancel` | Отменить талон (WAITING → CANCELLED) | ✅ Требуется | ADMIN, OPERATOR |
| 4 | `POST` | `/api/tickets/{ticketId}/move-backward` | Переместить талон на N шагов назад | ✅ Требуется | ADMIN, OPERATOR |
| 5 | `POST` | `/api/tickets/{ticketId}/move-to-position` | Переместить талон в целевую позицию | ✅ Требуется | ADMIN, OPERATOR |
| 6 | `GET` | `/api/tickets/{ticketId}/position` | Получить позицию талона в очереди | ✅ Требуется | ADMIN, OPERATOR |
| 7 | `GET` | `/api/tickets/queue` | Получить список ожидающих талонов (очереди) | ✅ Требуется | ADMIN, OPERATOR |

### Группа эндпоинтов `/api/tickets/all`

| # | HTTP Метод | Endpoint | Описание | Аутентификация | Разрешённые роли |
|---|------------|----------|----------|----------------|------------------|
| 8 | `GET` | `/api/tickets/all/` | Получить все талоны сессии | ✅ Требуется | ADMIN, OPERATOR |

### Матрица прав по ролям

| Роль | POST `/` (Создать) | GET `{id}` (Детали) | POST `{id}/cancel` (Отмена) | POST `{id}/move-backward` (Переместить назад) | POST `{id}/move-to-position` (Позиция) | GET `{id}/position` (Позиция) | GET `/queue` (Очередь) | GET `/all` (Все талоны) |
|------|---------------------|---------------------|-----------------------------|-----------------------------------------------|-----------------------------------------|-------------------------------|------------------------|-------------------------|
| **Неавторизованный** | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **Клиент** | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **EXECUTOR** | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
