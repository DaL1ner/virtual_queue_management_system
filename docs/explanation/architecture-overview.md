...
- Клиенту предоставляется возможность регистрации в очереди, отслеживания текущего статуса и положения, получения уведомлений и выхода из очереди через веб-интерфейс. Важно отметить, что добавление в очередь осуществляется без авторизации. Клиент не проходит процедуру аутентификации по учётным данным, а для идентификации в рамках текущей сессии указывает только имя и фамилию. Хранение профилей клиентов в рамках данного проекта не предусматривается. Таким образом сессия клиента и его присутствие в очереди жестко связаны, и при завершении обслуживания должна завершаться и сессия клиента в системе;  
...
### Архитектурное представление в нотации ArchiMate 3.2
...
![Общая диаграмма системы в нотации Archimate 3.2](diagrams/src/archimate/use-case/use-case.png)
