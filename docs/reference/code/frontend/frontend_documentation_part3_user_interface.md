...
#### 4. AdminView.vue
Интерфейс администратора с управлением пользователями, конфигурациями очередей, сессиями и типами услуг.

**Вкладки:**
- **Сессии очередей** — создание, открытие, пауза, закрытие сессий
- **Конфигурации** — управление конфигурациями очередей
- **Пользователи** — управление учётными записями сотрудников
- **Типы услуг** — управление типами услуг

**Управление пользователями:**
- **Создание пользователя** — форма с полями: логин, пароль, имя, фамилия, email, выбор ролей (чекбоксы)
- **Редактирование пользователя** — форма с полями: логин (только чтение), новый пароль (опционально), имя, фамилия, email, выбор ролей (чекбоксы)
- **Деактивация/активация** — кнопки для включения/отключения учётной записи

**Выбор ролей:**
- Список доступных ролей загружается с `GET /api/roles` при инициализации страницы
- Роли отображаются в виде группы чекбоксов с названием и кодом роли
- При редактировании текущие роли пользователя предзаполнены
- При сохранении выбранные `roleIds` отправляются на `PATCH /api/users/{id}`
...
Управляет состоянием административной панели.

**Ключевые данные:**
- `queueConfigs` - конфигурации очередей
- `queueSessions` - сессии очередей
- `users` - список пользователей
- `serviceTypes` - типы услуг
- `roles` - список доступных ролей (загружается с `GET /api/roles`)
...
- `updateUser(id, data)` - обновление пользователя (PATCH)
- `deactivateUser(id)` / `activateUser(id)` - деактивация/активация
- `fetchQueueConfigs()`, `createQueueConfig()`, `updateQueueConfig()`, `deleteQueueConfig()`
- `fetchQueueSessions()`, `createQueueSession()`, `changeSessionStatus()`
- `fetchServiceTypes()`, `createServiceType()`, `updateServiceType()`, `deleteServiceType()`
- `init()` - инициализация (загружает все данные)
- `startPolling()` / `stopPolling()` - периодическое обновление данных
